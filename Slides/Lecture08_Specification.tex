...
\begin{frame}[shrink=15]{Where We Are}
...
\begin{frame}{Functional Form Misspecification: Motivation \& Setup}

  \begin{highlightbox}
    \textbf{Motivation.} Even with \emph{every} relevant variable collected, OLS can
    be biased if $y$ is related to the $x_j$ through the \textbf{wrong functional
    form} --- a missing quadratic, log, or interaction. We \emph{have} the data; only
    the specification is wrong.
  \end{highlightbox}

  \begin{keybox}
    \textbf{Setup.} If the true model carries a nonlinear term we drop, that term is
    absorbed into the error and induces $\Cov(x_j, u) \neq 0$:
    \[
      y = \beta_0 + \beta_1 x_1 + \cdots + \beta_k x_k
        + \underbrace{\gamma\, x_1^2 + \cdots}_{\text{dropped}\;\to\; u} .
    \]
    \textbf{Plan:} (i) what misspecification \emph{is}, (ii) detect it --- $F$ test on
    quadratics and the \textbf{RESET} test, (iii) choose between \textbf{nonnested}
    models.
  \end{keybox}

\end{frame}

\begin{frame}[shrink=30]{Functional Form Misspecification: Definition}
  ...
  \begin{examplebox}{Example: A Wage Equation}
    True model:
    \[
      \log(\mathit{wage}) = \beta_0 + \beta_1 \mathit{educ} + \beta_2 \mathit{exper} + \beta_3 \mathit{exper}^2 + u.
    \]
    If we estimate
    \[
      \log(\mathit{wage}) = \beta_0 + \beta_1 \mathit{educ} + \beta_2 \mathit{exper} + u
    \]
  ...
  The \textbf{return to experience} is no longer $\beta_2$ but $\beta_2 + 2\beta_3 \mathit{exper}$,
...
\begin{frame}{Another Example: Missing Interactions}
  ...
  \[
    \log(\mathit{wage}) = \beta_0 + \beta_1 \mathit{educ} + \beta_2 \mathit{exper} + \beta_3 \mathit{exper}^2
    + \beta_4 \mathit{female} + \beta_5 (female \cdot educ) + u.
  \]
...
\begin{frame}[shrink=10]{Example 9.1: Economic Model of Crime (CRIME1)}
  ...
  Dependent variable: $\mathit{narr86}$ (number of arrests in 1986). $n = 2{,}725$.
  ...
  \hline
  $\mathit{pcnv}$ & $-0.133$ (0.040) & $0.553$ (0.154) \\
  $pcnv^2$ & --- & $-0.730$ (0.156) \\
  $\mathit{ptime86}$ & $-0.041$ (0.009) & $0.287$ (0.094) \\
  $ptime86^2$ & --- & $-0.0296$ (0.0039) \\
  $\mathit{inc86}$ & $-0.0015$ (0.0003) & $-0.0034$ (0.0008) \\
  ...
  \begin{keybox}
    \textbf{Interpretation:} The initial linear model overlooked important nonlinearities.
    But interpreting the quadratics is harder: e.g., $\mathit{pcnv}$ has a positive deterrent
    effect only up to $pcnv = 0.365$, then negative.
  \end{keybox}

\end{frame}

\begin{frame}[shrink=10]{The RESET Test}
...
\begin{frame}[shrink=10]{The RESET Statistic}
...
\begin{frame}{Example 9.2: Housing Price Equation (HPRICE1, $n = 88$)}
  ...
  \[
    price = \beta_0 + \beta_1 \mathit{lotsize} + \beta_2 \mathit{sqrft} + \beta_3 \mathit{bdrms} + u.
  \]

  \textbf{Model 2 (log--log):}
  \[
    \log(\mathit{price}) = \beta_0 + \beta_1 \log(\mathit{lotsize}) + \beta_2 \log(\mathit{sqrft}) + \beta_3 \mathit{bdrms} + u.
  \]
...
\begin{frame}{Caveats of Nonnested Testing}
  ...
  \begin{keybox}
    ...
    \begin{enumerate}\setlength{\itemsep}{2pt}
      ...
      \item \textbf{Rejecting A does not validate B:} Model A can be rejected for any
...
\begin{frame}{Proxy Variables: Motivation \& Setup}

  \begin{highlightbox}
    \textbf{Motivation.} The most damaging specification error is a \textbf{missing
    determinant we cannot observe} --- e.g.\ \emph{ability} in a wage equation. If it
    correlates with a regressor, OLS is biased and inconsistent (omitted-variable
    bias). Can we do anything \emph{without} the variable itself?
  \end{highlightbox}

  \begin{keybox}
    \textbf{Setup.} We cannot drop the unobservable (that \emph{is} the bias) and we
    cannot measure it. The compromise: substitute an observed \textbf{proxy} that is
    correlated with it. \textbf{Plan:} the proxy idea, the plug-in regression, the
    conditions under which it removes the bias, and using a \textbf{lagged dependent
    variable} as a proxy.
  \end{keybox}
...
\begin{frame}{Omitted Variables: A Bigger Problem}
  ...
  \begin{highlightbox}
    ...
    \[
      \log(\mathit{wage}) = \beta_0 + \beta_1 \mathit{educ} + \beta_2 \mathit{exper} + \beta_3 \mathit{abil} + u
    \]
    where $\mathit{abil}$ = ability — \textbf{unobservable}. If $\mathit{educ}$ is correlated with $\mathit{abil}$,
    putting $\mathit{abil}$ in the error causes \textbf{omitted variable bias}.
  \end{highlightbox}

  \textbf{Question:} Can we mitigate this bias if we cannot directly observe $\mathit{abil}$?

  \textbf{Answer:} Sometimes — using a \textbf{proxy variable}.

\end{frame}

\begin{frame}[shrink=20]{The Proxy Variable Idea}
...
\begin{frame}[shrink=15]{The Plug-in Solution}
...
\begin{frame}[shrink=10]{Why the Plug-in Solution Works}

  Plug the proxy equation $x_3^* = \delta_0 + \delta_3 x_3 + v_3$ into the model:
  \[
  ...
  \begin{highlightbox}
    ...
    \begin{itemize}\setlength{\itemsep}{1pt}
      ...
      \item \textbf{Cannot recover $\beta_3$ alone} (we do not observe $\delta_3$).
    \end{itemize}
  \end{highlightbox}
...
\begin{frame}[shrink=15]{Example 9.3: IQ as a Proxy for Ability (WAGE2)}
  ...
  \hline
  $\mathit{educ}$ & 0.065 (0.006) & 0.054 (0.007) & 0.053 (0.007) \\
  $\mathit{exper}$ & 0.014 (0.003) & 0.014 (0.003) & 0.014 (0.003) \\
  $\mathit{tenure}$ & 0.012 (0.002) & 0.011 (0.002) & 0.011 (0.002) \\
  $\mathit{IQ}$ & --- & 0.0036 (0.0010) & 0.0036 (0.0010) \\
...
\begin{frame}[shrink=10]{When the Proxy Is Imperfect}
...
\begin{frame}[shrink=15]{Lagged Dependent Variables as Proxies (\S9-2a)}

  \textbf{Sometimes we do not even have a candidate proxy.} A useful alternative:
  \textbf{the lagged dependent variable}.

  \begin{examplebox}{Example 9.4: City Crime Rates}
    \[
      crime = \beta_0 + \beta_1 \mathit{unem} + \beta_2 \mathit{expend} + \beta_3 \mathit{crime}_{-1} + u. \tag{9.16}
    \]
    \begin{itemize}\setlength{\itemsep}{1pt}
...
\begin{frame}[shrink=20]{Crime Rate Estimates (CRIME2, $n = 46$ cities)}
...
\begin{frame}[shrink=10]{Why Was Unemployment Negative Without the Lag?}

  Theory predicts $\beta_{\mathit{unem}} \ge 0$ (joblessness raises crime), yet column~(1)
  reports $-0.029$. Two points resolve the puzzle.

  \begin{keybox}
    \textbf{1. It is not significant.} $t = -0.029/0.032 \approx -0.9$ --- the negative
    estimate is \textbf{statistically indistinguishable from zero}. The ``wrong sign'' is
    sampling noise, not a finding: never over-read a single insignificant coefficient.
  \end{keybox}

  \begin{highlightbox}
    \textbf{2. Omitted-variable bias.} Column~(1) leaves out \textbf{persistent city
    traits} (demographics, socioeconomic conditions, reporting practices) that drive
    crime; $\log(crmrte_{82})$ proxies for them. With $\delta$ = effect of those traits on
    crime and $\gamma$ = their partial association with $\mathit{unem}$,
    \[
      \plim \hat\beta_{\mathit{unem}}^{(1)} = \beta_{\mathit{unem}} + \delta\,\gamma .
    \]
    The coefficient \emph{rises} ($-0.029 \to +0.009$) once the lag is added, so the bias
    $\delta\gamma$ was \textbf{negative}. Since $\delta>0$ (past crime enters at $+1.19$),
    we infer $\gamma<0$: in this sample, higher-baseline-crime cities had \emph{lower}
    1987 unemployment.
  \end{highlightbox}

  \textbf{After the lag:} the confounding is absorbed and $\hat\beta_{\mathit{unem}}$ returns
  to its expected non-negative sign ($+0.009$) --- though still insignificant at $n=46$.

\end{frame}

\begin{frame}[shrink=20]{A Different Slant on Multiple Regression (\S9-2b)}
  ...
  \begin{highlightbox}
    ...
    \begin{itemize}\setlength{\itemsep}{1pt}
      ...
      \item Do not include variables \textbf{unobserved at the time of prediction}.
    \end{itemize}
  \end{highlightbox}
...
\begin{frame}[shrink=12]{Potential Outcomes and Proxy Variables (\S9-2c)}

  The proxy-variable idea links directly to the \textbf{potential outcomes} framework
  (Ch.\ 2--7). For a binary treatment $w \in \{0,1\}$ with potential outcomes
  $y(0), y(1)$:
  \[
    y(0) = \mu_0 + v(0), \qquad y(1) = \mu_1 + v(1), \qquad
    \tau_{\mathit{ate}} = \mu_1 - \mu_0 .
  \]

  \begin{keybox}
    \textbf{Selection problem:} participation $w$ may be correlated with the
    unobservables $v(0), v(1)$. \textbf{Unconfoundedness (ignorability):} conditional
    on covariates $\mathbf{x}$, $w$ is independent of $\bigl(y(0), y(1)\bigr)$ ---
    which is \emph{exactly the proxy-variable condition}: $\mathbf{x}$ proxies for the
    unobserved factors driving both the outcome and the treatment decision.
  \end{keybox}

  \begin{highlightbox}
    \textbf{Estimation.} With linear forms
    $\E[v(g)\mid w,\mathbf{x}] = \E[v(g)\mid \mathbf{x}]
      = (\mathbf{x}-\boldsymbol\eta)\boldsymbol\beta_g$, $\boldsymbol\eta = \E(\mathbf{x})$,
    the ATE comes from one interacted regression over the \emph{entire} sample,
    \[
      y_i \;\text{ on }\; w_i,\; \mathbf{x}_i,\; w_i\cdot(\mathbf{x}_i - \bar{\mathbf{x}}),
    \]
    where the coefficient on $w_i$ is $\hat\tau_{\mathit{ate}}$ (derived next, \S7-6).
  \end{highlightbox}

\end{frame}

\begin{frame}[shrink=10]{The Parameter of Interest and the ATE Regression (\S7-6)}

  \begin{keybox}
    \textbf{Setup.} Each unit has potential outcomes $y(1)$ (if treated) and $y(0)$ (if
    untreated); with treatment $w\in\{0,1\}$ we observe only one,
    $y = (1-w)\,y(0) + w\,y(1)$, where $y(g)=\mu_g+v(g)$ and $\mu_g=\E[y(g)]$.

    \smallskip
    \textbf{Parameter of interest --- the average treatment effect (ATE):}
    \[
      \tau \;=\; \E[\,y(1)-y(0)\,] \;=\; \mu_1-\mu_0 .
    \]
    The \emph{unit-level} effect $y_i(1)-y_i(0)$ is never observed, so we target its average.
  \end{keybox}

  \textbf{Derivation.} Take $\E(\cdot\mid w,\mathbf{x})$; \textbf{unconfoundedness} drops $w$
  from each conditional mean:
  \begin{align*}
    \E(y \mid w, \mathbf{x})
      &= \E[y(0)\mid\mathbf{x}] + w\,\big\{\E[y(1)\mid\mathbf{x}] - \E[y(0)\mid\mathbf{x}]\big\} \\
      &= \mu_0 + (\mathbf{x}-\boldsymbol\eta)\boldsymbol\beta_0
         + \underbrace{(\mu_1-\mu_0)}_{\tau}\,w
         + (\boldsymbol\beta_1-\boldsymbol\beta_0)\,w(\mathbf{x}-\boldsymbol\eta).
  \end{align*}
  $\Rightarrow$ regress $y$ on $w$, $\mathbf{x}$, $w(\mathbf{x}-\bar{\mathbf{x}})$
  ($\boldsymbol\eta=\E(\mathbf{x})$ replaced by $\bar{\mathbf{x}}$); the coefficient on $w$
  estimates $\tau$.

\end{frame}

\begin{frame}{Why That Form? Interaction and Centering (\S7-6)}

  \begin{keybox}
    \textbf{Why the interaction $w(\mathbf{x}-\bar{\mathbf{x}})$?} It lets the treated and
    control groups have \emph{different slopes} ($\boldsymbol\beta_1\neq\boldsymbol\beta_0$),
    so the effect may vary with covariates:
    \[
      \tau(\mathbf{x}) = \tau + (\mathbf{x}-\boldsymbol\eta)(\boldsymbol\beta_1-\boldsymbol\beta_0).
    \]
    Dropping it forces a \emph{constant} effect $\tau(\mathbf{x})\equiv\tau$.
  \end{keybox}

  \begin{highlightbox}
    \textbf{Why \emph{center} at $\bar{\mathbf{x}}$?} The ATE is the \emph{average} of
    $\tau(\mathbf{x})$, and $\E(\mathbf{x}-\boldsymbol\eta)=0$, so the coefficient on $w$
    equals $\tau$ \textbf{only when the interaction is demeaned}. With a raw $w\mathbf{x}$
    term, that coefficient would instead be the effect at $\mathbf{x}=\mathbf{0}$ --- a
    (usually fictional) unit, not the population average.
  \end{highlightbox}
...
\begin{frame}{Models with Random Slopes: Motivation \& Setup}

  \begin{highlightbox}
    \textbf{Motivation.} Every model so far forces \emph{one} slope $\beta_j$ on the
    whole population. But the return to education, or the effect of a price change,
    plausibly \textbf{differs across individuals} by unobserved traits. What is OLS
    estimating when slopes are heterogeneous?
  \end{highlightbox}

  \begin{keybox}
    \textbf{Setup.} Allow each unit its own intercept $a_i$ and slope $b_i$, and
    define the population averages $\alpha = \E(a_i)$ and $\beta = \E(b_i)$, so that
    $\beta$ is the \textbf{average partial effect (APE)}. \textbf{Plan:} the
    conditions under which OLS still recovers $\alpha,\beta$, the
    heteroskedasticity it induces, and when it breaks down.
  \end{keybox}

\end{frame}

\begin{frame}[shrink=25]{The Random Slopes Model}
...
\begin{frame}[shrink=20]{When OLS Still Works}
...
\begin{frame}[shrink=5]{Random Slopes: Practical Takeaway}
...
\begin{frame}{Measurement Error: Motivation \& Setup}

  \begin{highlightbox}
    \textbf{Motivation.} We often record a variable \emph{with error}: reported vs
    true income, average vs marginal tax rate, self-reported vs actual hours. Does a
    mismeasured variable bias OLS --- and if so, in \textbf{which direction}?
  \end{highlightbox}

  \begin{keybox}
    \textbf{Setup.} Write each observed value as truth plus noise, e.g.\
    $x_1 = x_1^{*} + e_1$, and trace how the noise $e_1$ propagates into
    $\hat{\boldsymbol\beta}$. \textbf{Plan:} (i) measurement error in the
    \emph{dependent} variable (usually harmless), then (ii) error in an
    \emph{explanatory} variable under the \textbf{classical errors-in-variables}
    assumption $\Rightarrow$ \textbf{attenuation bias} toward zero.
  \end{keybox}

\end{frame}

\begin{frame}[shrink=20]{Measurement Error: The Issue}
...
\begin{frame}[shrink=15]{Measurement Error in the Dependent Variable (\S9-4a)}
...
\begin{frame}[shrink=25]{Example 9.5: Savings Function}

  Consider a savings function:
  \[
    sav^* = \beta_0 + \beta_1 \mathit{inc} + \beta_2 \mathit{size} + \beta_3 \mathit{educ} + \beta_4 \mathit{age} + u.
  \]

  \begin{itemize}\setlength{\itemsep}{2pt}
    \item Observed savings $\mathit{sav}$ may differ from true $sav^*$ due to reporting error.
...
\begin{frame}[shrink=30]{Measurement Error in an Explanatory Variable (\S9-4b)}
...
\begin{frame}[shrink=15]{The Attenuation Bias under CEV}
  ...
  \[
    y = \beta_0 + \beta_1 x_1 + (u - \beta_1 e_1). \tag{9.32}
  \]
...
\begin{frame}[shrink=10]{How Big Is the Attenuation?}
...
\begin{frame}[shrink=15]{Multiple Regression with CEV}
...
\begin{frame}[shrink=15]{Example 9.7: GPA Equation with Measurement Error}

  Suppose true model:
  \[
    colGPA = \beta_0 + \beta_1 \mathit{faminc}^* + \beta_2 \mathit{hsGPA} + \beta_3 \mathit{SAT} + u,
  \]
...
\begin{frame}{Data Problems: Motivation \& Setup}

  \begin{highlightbox}
    \textbf{Motivation.} Real samples are imperfect: observations have \textbf{gaps},
    the sample may be drawn \textbf{nonrandomly}, and a few \textbf{outliers} can
    dominate the fit. Each can threaten random sampling (MLR.2) --- but not all of
    them bias OLS.
  \end{highlightbox}

  \begin{keybox}
    \textbf{Setup.} For every data problem the decisive question is the same:
    \emph{is an observation's inclusion or exclusion related to the error $u$?}
    \begin{itemize}\setlength{\itemsep}{1pt}
      \item \textbf{Missing data} (§9-5a): MCAR / MAR / NMAR; complete-case vs.\ the
        missing-indicator method.
      \item \textbf{Nonrandom samples} (§9-5b): \emph{exogenous} (harmless) vs.\
        \emph{endogenous} (biasing) selection.
      \item \textbf{Outliers} (§9-5c): influential observations --- which motivate
        \textbf{LAD} in §9-6.
    \end{itemize}
  \end{keybox}
...
\begin{frame}[shrink=15]{Missing Data (\S9-5a)}
  ...
  \textbf{Example:} If 196 of 1{,}388 observations in BWGHT are missing $\mathit{faminc}$, simply drop
  them. As long as missingness is unrelated to factors affecting $y$, the remaining sample
  is still random.

\end{frame}

\begin{frame}[shrink=15]{The Missing Indicator Method (MIM)}
...
\begin{frame}[shrink=8]{Why MIM Needs a Strong Assumption}

  Zero-imputing $x_k$ makes the fitted model differ across the two groups:
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{observed} ($m_k=0$):
      $y = \beta_0 + \sum_{j<k}\beta_j x_j + \beta_k x_k + u$ \quad(the true model);
    \item \textbf{missing} ($m_k=1$):
      $y = (\beta_0+\gamma) + \sum_{j<k}\beta_j x_j + u$ \quad($\beta_k x_k$ dropped,
      replaced by the dummy $\gamma$).
  \end{itemize}

  \begin{keybox}
    For the missing rows MIM \textbf{throws away} $\beta_k x_k$ and lets the single dummy
    $\gamma$ stand in for it. A dummy can absorb only a \textbf{level shift} --- the
    \emph{average} of the dropped term, $\gamma \approx \beta_k\,\E(x_k \mid \text{missing})$.
  \end{keybox}

  \begin{highlightbox}
    This works \textbf{only if} $\E(x_k \mid x_1,\dots,x_{k-1},\text{missing})$ does not
    move with the other regressors --- i.e.\ $x_k$ is \textbf{uncorrelated} with them. If
    $x_k$ \emph{is} correlated with $x_1,\dots,x_{k-1}$, the dropped $\beta_k x_k$ varies
    with those regressors and is mis-attributed to $\beta_1,\dots,\beta_{k-1}$ ---
    omitted-variable bias the dummy cannot fix (it shifts the \emph{intercept}, not the
    slopes). \textbf{Contrast:} complete-case OLS is consistent under MCAR \emph{without}
    this condition --- it just uses fewer observations.
  \end{highlightbox}

\end{frame}

\begin{frame}{Why MIM Needs a Strong Assumption: The Algebra}

  Use the identity $x_k = z_k + m_k x_k$ (observed $\Rightarrow m_k = 0,\ x_k = z_k$;
  missing $\Rightarrow z_k = 0$). Substituting into the true model
  $y = \beta_0 + \sum_{j<k}\beta_j x_j + \beta_k x_k + u$,
  \[
    \text{(true model)}\quad
    y = \beta_0 + \sum_{j<k}\beta_j x_j + \beta_k z_k
        + \underbrace{\beta_k\, m_k x_k}_{\text{omitted by MIM}} + u .
  \]
  MIM instead fits the linear specification
  \[
    \text{(MIM fits)}\quad
    \hat{y} = \hat\gamma_0 + \sum_{j<k}\hat\gamma_j x_j + \hat\gamma_k z_k
              + \hat\gamma_m\, m_k ,
  \]
  replacing the true term $\beta_k(m_k x_k)$ with a plain dummy $\hat\gamma_m m_k$ --- a
  \emph{misspecified} form that drops the $m_k x_k$ interaction.

\end{frame}

\begin{frame}{Why a Dummy Catches Only the Mean}

  Treat MIM as a misspecified model whose error \textbf{absorbs} the dropped term,
  $v = u + \beta_k m_k x_k - \gamma_m m_k$. OLS forces the residual orthogonal to the
  dummy, $\E(v\,m_k)=0$. Using $m_k^2 = m_k$ and $\E(u\,m_k)=0$,
  \[
    \beta_k\,\E(m_k x_k) - \gamma_m\,\E(m_k) = 0
    \;\Longrightarrow\;
    \gamma_m = \beta_k\,\frac{\E(m_k x_k)}{\E(m_k)} = \beta_k\,\E(x_k\mid\text{miss}) .
  \]

  \begin{keybox}
    The $m_k$ normal equation \textbf{pins the dummy to the group mean}. Splitting the
    dropped term accordingly,
    \[
      \beta_k x_k
      = \underbrace{\beta_k\,\E(x_k\mid\text{miss})}_{\text{absorbed by }\gamma_m}
      + \underbrace{\beta_k\bigl(x_k - \E(x_k\mid\text{miss})\bigr)}_{\text{varies within the group}} ,
    \]
    only the constant piece fits a dummy --- the deviation needs the omitted $m_k x_k$
    (the source of the slope bias; next slide).
  \end{keybox}

\end{frame}

\begin{frame}{MIM Bias: Where the Leftover Goes}

  Whether that leftover deviation biases the slopes depends on how $x_k$ relates to the
  other regressors. Model the missing-data conditional mean as linear:
  \[
    \E(x_k \mid x_1, \ldots, x_{k-1},\, m_k = 1)
      = \delta_0 + \textstyle\sum_{j<k}\delta_j x_j .
  \]

  \begin{keybox}
    The dummy absorbs the constant $\gamma = \beta_k\delta_0$, but each
    $\beta_k\delta_j x_j$ is collinear with $x_j$ --- so it contaminates $\hat\beta_j$.
  \end{keybox}

  \begin{highlightbox}
    Hence $\;\plim \hat\beta_j^{\text{MIM}} = \beta_j + (\text{a term} \propto
    \beta_k\delta_j)$. The bias vanishes for \emph{all} $j$ \;$\iff$\;
    $\delta_1 = \cdots = \delta_{k-1} = 0$ \;$\iff$\; $x_k$ is \textbf{uncorrelated} with
    $x_1, \ldots, x_{k-1}$ in the missing subpopulation. Otherwise the slopes are biased.
  \end{highlightbox}
...
\begin{frame}[shrink=10]{Nonrandom Samples (\S9-5b)}
...
\begin{frame}[shrink=25]{Sample Selection: Examples}

  \begin{examplebox}{Exogenous Selection: Saving Study}
    \[
      saving = \beta_0 + \beta_1 \mathit{income} + \beta_2 \mathit{age} + \beta_3 \mathit{size} + u.
    \]
    Sample only people over 35. Selection is on $\mathit{age}$ (a regressor). \textbf{OLS is
    unbiased.} Just a smaller, less general sample.
  \end{examplebox}

  \begin{examplebox}{Endogenous Selection: Wealth Equation}
    \[
      wealth = \beta_0 + \beta_1 \mathit{educ} + \beta_2 \mathit{exper} + \beta_3 \mathit{age} + u.
    \]
...
\begin{frame}{Wage Equations and Working Samples}
  ...
  \[
    \log(wage^o) = \beta_0 + \beta_1 \mathit{educ} + \beta_2 \mathit{exper} + u, \tag{9.39}
  \]
...
\begin{frame}[shrink=5]{Outliers and Influential Observations (\S9-5c)}
...
\begin{frame}[shrink=5]{Example 9.8: R\&D Intensity and Firm Size (RDCHEM)}

  Data: 32 chemical companies, $\mathit{rdintens}$ = R\&D as \% of sales, $\mathit{sales}$ in millions.
  ...
  \]
  with standard errors 0.586, 0.000044, 0.0462. $R^2 = 0.076$. \textbf{No coefficient is
  significant at 10\%.}
  ...
  \begin{highlightbox}
    \textbf{Striking result:} Coefficient on $\mathit{sales}$ \textbf{more than triples}, becomes
    \textbf{significant at 5\%}. The single large firm was masking the relationship.
  \end{highlightbox}

\end{frame}

\begin{frame}[shrink=10]{Detecting Outliers: Studentized Residuals}
...
\begin{frame}[shrink=15]{Studentized Residuals: Caveats}
  ...
  \begin{itemize}\setlength{\itemsep}{1pt}
    ...
    \item \textbf{Do not just delete} unless the observation is clearly a data entry error.
  \end{itemize}

\end{frame}

\begin{frame}[shrink=15]{Example 9.10: Infant Mortality (INFMRT)}

  $n = 51$ states + DC. All variables in logs:
  \[
    \widehat{infmort} = 33.86 - 4.68 \log(\mathit{pcinc}) + 4.15 \log(\mathit{physic}) - 0.088 \log(\mathit{popul}).
  \]
  ...
  \textbf{Drop DC:}
  \[
    \widehat{infmort} = 23.95 - 0.57 \log(\mathit{pcinc}) - 2.74 \log(\mathit{physic}) + 0.629 \log(\mathit{popul}).
  \]
  Now $\log(\mathit{physic})$ has the \textbf{expected negative sign} and is significant.
...
\begin{frame}[shrink=20]{Functional Form and Outlier Sensitivity}
  ...
  \begin{keybox}
    ...
    \medskip
    \textbf{Example 9.9 (RDCHEM in logs):} Re-estimate the R\&D model in log--log:
    \[
      \log(\mathit{rd}) = \beta_0 + \beta_1 \log(\mathit{sales}) + \beta_2 \mathit{profmarg} + u.
    \]
  \end{keybox}

  \textbf{All 32 firms:}
  \[
    \widehat{\log(\mathit{rd})} = -4.378 + 1.084 \log(\mathit{sales}) + 0.0217\,profmarg,\ R^2 = 0.918.
  \]
  \textbf{Drop largest firm:}
  \[
    \widehat{\log(\mathit{rd})} = -4.404 + 1.088 \log(\mathit{sales}) + 0.0218\,profmarg,\ R^2 = 0.904.
  \]
...
\begin{frame}{Least Absolute Deviations: Motivation \& Setup}

  \begin{highlightbox}
    \textbf{Motivation.} OLS minimizes \emph{squared} residuals, so a single extreme
    $y$ can swing every coefficient. Rather than hunt down and delete outliers, can
    we estimate with a rule that is \textbf{intrinsically resistant} to them?
  \end{highlightbox}

  \begin{keybox}
    \textbf{Setup.} Replace OLS's quadratic loss with an \textbf{absolute-value}
    loss --- minimize the sum of \emph{absolute} residuals rather than squared ones.
    This estimates the \textbf{conditional median} instead of the mean.
    \textbf{Plan:} the LAD criterion, why the median is robust to outliers, and the
    robustness--efficiency trade-off against OLS.
  \end{keybox}
...
\begin{frame}[shrink=20]{LAD vs.\ OLS: Visual Comparison}
...
\begin{frame}[shrink=10]{LAD Estimates the Conditional Median}
...
\begin{frame}[shrink=20]{LAD: Computation and Inference}
...
\begin{frame}{When to Choose LAD over OLS}
  ...
  \hline
  No outliers, symmetric errors & \textcolor{green!60!black}{\checkmark} (more efficient) & — \\
  Outliers in $y$ & risky & \textcolor{green!60!black}{\checkmark} (robust) \\
  Asymmetric errors & estimates mean & estimates median \\
  Skewed dependent variable & risky & \textcolor{green!60!black}{\checkmark} (often more interpretable) \\
  Need exact small-sample inference & \textcolor{green!60!black}{\checkmark} & \textcolor{red!70!black}{$\times$} \\
  Large samples & \textcolor{green!60!black}{\checkmark} & \textcolor{green!60!black}{\checkmark} \\
  Mean is the policy quantity & \textcolor{green!60!black}{\checkmark} & \textcolor{red!70!black}{$\times$} \\
  Median is the policy quantity & \textcolor{red!70!black}{$\times$} & \textcolor{green!60!black}{\checkmark} \\
  \hline
  \end{tabular}
  \end{center}
...
\begin{frame}[shrink=25]{Quantile Regression Generalization}
...
\begin{frame}[shrink=20]{Chapter 9 Summary}
...
\begin{frame}[shrink=15]{Looking Ahead}
