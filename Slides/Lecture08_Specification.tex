...
\begin{frame}{Why a Dummy Catches Only the Mean}
  ...
  The dummy $m_k$ can add only a \emph{constant} to every missing row, and least squares
  makes that constant the group \emph{mean} (the best constant satisfies
  $\arg\min_{c}\E[(W-c)^2] = \E(W)$).

  \begin{keybox}
    Split the dropped term into its missing-group mean plus deviations:
    \[
      \beta_k x_k
      = \underbrace{\beta_k\,\E(x_k\mid\text{miss})}_{\text{constant}\;\to\;\gamma}
      + \underbrace{\beta_k\bigl(x_k - \E(x_k\mid\text{miss})\bigr)}_{\text{varies within the group}} .
    \]
    Only the first piece fits a dummy; capturing the second would need the omitted
    interaction $m_k x_k$.
  \end{keybox}
