...
\begin{frame}{Why MIM Needs a Strong Assumption: The Algebra}
  ...
  \]
  so MIM includes the dummy $m_k$ but \emph{omits} $\beta_k m_k x_k$.

  \begin{keybox}
    \textbf{Why a dummy catches only the mean.} A dummy adds one constant to every
    missing row, and least squares makes that constant the group \emph{mean} --- the best
    constant satisfies $\arg\min_{c}\E[(W-c)^2] = \E(W)$. Split the dropped term:
    \[
      \beta_k x_k
