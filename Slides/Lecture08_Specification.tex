...
\begin{frame}[shrink=8]{Why MIM Needs a Strong Assumption: The Algebra}
  ...
  $y = \beta_0 + \sum_{j<k}\beta_j x_j + \beta_k x_k + u$ gives
  \[
    y = \beta_0 + \sum_{j<k}\beta_j x_j + \beta_k z_k
        + \underbrace{\beta_k\, m_k x_k}_{\text{\textbf{not} a MIM regressor}} + u .
  \]
  MIM regresses $y$ on $1, x_1, \ldots, x_{k-1}, z_k, m_k$: it includes $m_k$ but
  \emph{omits} the term $\beta_k m_k x_k$.

  \begin{keybox}
    Model the missing-data conditional mean of $x_k$ as linear:
    \[
      \E(x_k \mid x_1, \ldots, x_{k-1},\, m_k = 1)
        = \delta_0 + \textstyle\sum_{j<k}\delta_j x_j .
    \]
    On the missing rows the omitted term is $\beta_k\bigl(\delta_0 + \sum_{j<k}\delta_j
    x_j\bigr)$: the dummy soaks up the constant $\gamma = \beta_k\delta_0$, but each
    $\beta_k\delta_j x_j$ is collinear with the regressor $x_j$.
  \end{keybox}
