...
\begin{frame}[shrink=25]{A Single Dummy: The Gender-Wage Model}
  ...
  \begin{itemize}
    \item $\beta_0$ = hourly wage for a \textbf{male} with 0 education (intercept)
    \item $\delta_0$ = \textbf{difference in wage} between females and males, holding education fixed
    \item $\beta_1$ = return to one additional year of education (for both men and women)
  \end{itemize}
...
\begin{frame}[shrink=25]{The Dummy Variable Trap}
  ...
  Result: OLS cannot estimate both $\beta_0$ and $\delta_{\text{male}}$ uniquely (singular matrix).
...
\begin{frame}[shrink=30]{Example 7.1: Gender Wage Gap Regression}
  ...
  \begin{keybox}
    \begin{itemize}
      ...
      \item This is the \textbf{controlled gender wage gap} or \textbf{conditional} gap.
      \item t-statistic for female: $t = -1.81 / 0.26 = -6.96$ (highly significant).
      \item Causal interpretation requires assuming no omitted variables correlated with both female and wage.
    \end{itemize}
  \end{keybox}
...
\begin{frame}[shrink=28]{Example 7.8: Law School Rankings and Salaries}
  ...
  \begin{keybox}
    \begin{itemize}
      \item Base group: schools ranked outside top 100.
      \item Top-10 schools: $\exp(0.700) - 1 \approx +1.01$ (approximately double salary!).
      \item Ranks 11-25: $\exp(0.594) - 1 \approx +0.81$ (roughly double).
      \item Clearly, law school ranking has enormous effect on starting salary.
    \end{itemize}
  \end{keybox}
...
\begin{frame}[shrink=25]{The Chow Test: Comparing Regression Functions}
  ...
  \textbf{Null hypothesis:} $H_0: \delta_0 = \delta_1 = \cdots = \delta_k = 0$ (same regression for both groups).

  \textbf{Under} $H_0$: $F \sim F_{k+1, n-2(k+1)}$.
...
\begin{frame}[shrink=20]{Binary Dependent Variables: Motivation}
  ...
  \begin{examplebox}{Examples}
    \begin{itemize}
      \item Arrested or not arrested (1 = arrested)
      \item Employed or not (1 = employed)
      \item Home ownership: yes or no (1 = owner)
      \item College attendance: yes or no (1 = attended)
      \item Loan approval: approved or denied (1 = approved)
    \end{itemize}
  \end{examplebox}
  ...
  \begin{keybox}
    Yes! The model makes sense: $\E[y | x]$ = predicted probability of success.

    But OLS has limitations when $y$ is binary (we'll explore these next).
  \end{keybox}
...
\begin{frame}[shrink=25]{The Linear Probability Model (LPM)}
  ...
  \begin{definitionbox}{LPM Definition}
    \[
      y = \beta_0 + \beta_1 x_1 + \cdots + \beta_k x_k + u \quad \text{(eq.\ 7.27)}
    \]

    where $y \in \{0, 1\}$ (binary).

    OLS estimates the conditional probability:
    \[
  ...
  \textbf{Slope interpretation:} $\beta_j$ measures the change in \textbf{probability of success} when $x_j$ increases by one unit.
  ...
  \begin{highlightbox}
    ...
    Example: If $\beta_j = 0.05$ and $x_j$ increases by 1, the probability of success \textit{increases by 5 percentage points}.
  \end{highlightbox}
...
\begin{frame}[shrink=20]{LPM Drawbacks}
  ...
  \begin{enumerate}
    \item \textbf{Predicted probabilities can fall outside [0, 1].}
    \begin{itemize}
      \item If $\hat{y} = \hat{\beta}_0 + \hat{\beta}_1 x_1 + \cdots < 0$ or $> 1$, the prediction is nonsensical.
      \item Nonlinear models (Probit, Logit) constrain predictions to [0, 1] automatically.
    \end{itemize}

    \vspace{0.2cm}

    \item \textbf{The marginal effect must be constant.}
    \begin{itemize}
      \item Probability changes linearly in $x$, but logically it should be slower at extremes.
      \item True effects often have diminishing returns: harder to move probability from 0.9 to 1.0 than from 0.5 to 0.6.
    \end{itemize}

    \vspace{0.2cm}

    \item \textbf{Inherent heteroskedasticity:}
    \[
      \Var(u | x) = p(x) \cdot [1 - p(x)] \quad \text{(eq.\ 7.30)}
    \]
    \begin{itemize}
      \item Variance depends on $x$ (not constant $\sigma^2$, so MLR.5 is violated).
      \item Standard OLS standard errors are wrong; use robust SEs instead.
    \end{itemize}
  \end{enumerate}
...
\begin{frame}[shrink=20]{Program Evaluation: The Fundamental Problem}
  ...
  \textbf{Challenge:} \textbf{Self-selection bias.}

  \begin{keybox}
    \begin{itemize}
      \item Workers who volunteer for training may be more motivated or have higher ability.
      \item Simply comparing treated and untreated workers confounds the program effect with pre-existing differences.
      \item Need regression adjustment to \textit{control} for observable differences.
    \end{itemize}
  \end{keybox}
