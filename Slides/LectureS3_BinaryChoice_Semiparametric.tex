...
\begin{frame}{二元选择中的半参数动机}
  ...
  \begin{highlightbox}
    \textbf{若误差分布错设定：}
    \begin{itemize}\setlength{\itemsep}{1pt}
      \item Probit MLE $\hat\beta_{\text{Probit}}$ 不一致（除非恰好 $G = \Phi$）
      \item 边际效应 APE 也不一致
...
\begin{frame}[fragile]{实际计算细节}
  ...
  # Binary-choice Ichimura via np::npindex
  bw <- npindexbw(formula = y ~ x1 + x2 + x3,
                  method = "ichimura",
                  data = mroz_data)
  fit <- npindex(bws = bw)
  summary(fit)  # output: hat beta + asymptotic variance
  \end{verbatim}
  \normalsize
...
%% ----- Frame 40 -----
\begin{frame}{当 $G = \Phi$ 时：Ichimura 与 Probit 的效率对比}
  ...
  方法 & 一致性 & 渐近正态性 & 渐近方差 \\
  \hline
  Probit MLE & \textcolor{green!60!black}{\checkmark} & \textcolor{green!60!black}{\checkmark} & 达到 Cramér-Rao 下界 $V^{*}_{P}$ \\
  \hline
  Ichimura SLS & \textcolor{green!60!black}{\checkmark} & \textcolor{green!60!black}{\checkmark} & $V^{*}_{I} \succeq V^{*}_{P}$（可能严格大于）\\
  \hline
  \end{tabular}
  \end{center}
...
\begin{frame}{当 $G \neq \Phi$ 时：Probit 的偏误 vs Ichimura}
  ...
  方法 & 一致性 & 渐近方差 & 性质 \\
  \hline
  Probit MLE & \textcolor{red}{\textbf{\textcolor{red!70!black}{$\times$}}} & \textcolor{red}{无意义} & $\hat\beta_P \to \beta^{*} \neq \beta_0$（伪真值）\\
  \hline
  Ichimura SLS & \textbf{\textcolor{green!60!black}{\checkmark}} & 正常 $V^{-1}\Sigma V^{-1}$ & 始终一致，分布不依赖 $G$ 设定 \\
  \hline
  \end{tabular}
  \end{center}
...
\begin{frame}{Klein-Spady vs Probit：何时使用？}
  ...
  \hline
  $G$ 真为 $\Phi$ & \textcolor{green!60!black}{\checkmark} 效率最高 & 一致但有 1\%-30\% 效率损失 \\
  \hline
...
\begin{frame}{Han MRC 与广义模型的统一框架}
  ...
  模型 & 设定 & MRC 适用 \\
  \hline
  二元选择 & $Y = \mathbf{1}\{X'\beta + \varepsilon > 0\}$ & \textcolor{green!60!black}{\checkmark} \\
  \hline
  归并/受限因变量 & $Y = \max(0, X'\beta + \varepsilon)$（Tobit）& \textcolor{green!60!black}{\checkmark} \\
  \hline
  有序响应 & $Y \in \{0, 1, \ldots, J\}$，阈值未知 & \textcolor{green!60!black}{\checkmark} \\
  \hline
  截尾回归 & $Y = X'\beta + \varepsilon$ 截断观测 & \textcolor{green!60!black}{\checkmark} \\
  \hline
  广义半参数 & $Y = D(X'\beta + \varepsilon)$，$D$ 单调 & \textcolor{green!60!black}{\checkmark} \\
  \hline
  \end{tabular}
  \end{center}
...
\begin{frame}{Manski vs Horowitz：直接对比}
  ...
  \hline
  异方差稳健 & \textcolor{green!60!black}{\checkmark} & \textcolor{green!60!black}{\checkmark} \\
  \hline
  收敛速度 & $n^{-1/3}$ & $n^{-2/5}$（更快）\\
  \hline
  极限分布 & 非正态 & \textbf{正态} \\
...
\begin{frame}[shrink=15]{Lewbel 估计量构造}
  ...
  \begin{highlightbox}
    \textbf{这是非平凡的结果。}变换 $T_i$ 是 $X_{1i}'\beta_0$ 的\textbf{线性条件期望}——
    即使原模型有任意异方差。意味着：
    \begin{itemize}\setlength{\itemsep}{1pt}
...
# Probit baseline
probit_fit <- glm(inlf ~ nwifeinc + educ + exper + I(exper^2) +
                  age + kidslt6 + kidsge6,
                  data = mroz, family = binomial(link = "probit"))

# Klein-Spady (np package)
library(np)
ks_bw <- npindexbw(formula = inlf ~ nwifeinc + educ + exper +
