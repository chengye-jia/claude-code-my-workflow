...
\begin{frame}{本讲路线图}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \textbf{第五部分}
      \begin{itemize}\footnotesize
        \item §8 非参数推断（置信区间与 Wild Bootstrap）
      \end{itemize}
      \textbf{第六部分}
      \begin{itemize}\footnotesize
        \item §9 级数方法（Fourier、Hermite、样条）
      \end{itemize}
      \textbf{第七部分}
      \begin{itemize}\footnotesize
        \item §10 扩展：离散 \& 混合数据核
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{highlightbox}
        ...
        \]
        \textbf{为什么快？}所有 $n$ 个观测值都在估计\textbf{同一个}参数 $\beta$——每个 $X_i$ 都携带 $\beta$ 的信息。
      \end{highlightbox}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{highlightbox}
        \textbf{非参数收敛速度 $n^{-2/(4+d)}$：}
        \[
          \hat m(x_0) - m(x_0) = O_p(n^{-2/(4+d)})
        \]
        \textbf{为什么慢？}只有 $X_i$ 靠近 $x_0$ 的观测才对 $m(x_0)$ 有用——\textbf{有效样本量}约为 $nh^d$。
      \end{highlightbox}
    \end{column}
  \end{columns}
...
\begin{frame}{核函数的形式定义与条件}
  ...
  \begin{definitionbox}{{核函数 (Kernel) — Li \& Racine §1.4}}
    ...
    \begin{align*}
      ...
      \text{(K4)} \quad & R(K) \;\equiv\; \int K(u)^2\, du < \infty && \text{(粗糙度有限)}
    \end{align*}
  \end{definitionbox}
...
\begin{frame}{MSE 与 IMSE：从逐点到积分}
  ...
  \]
  其中 $R(f'') = \int [f''(x)]^2 dx$ 衡量密度的\textbf{粗糙度}（振荡越多，$R(f'')$ 越大）。
...
\begin{frame}{局部二次与更高阶多项式}
  ...
  \begin{keybox}
    ...
    \begin{itemize}
      ...
      \item $p\geq 3$ 的额外收益通常被方差增加抵消——\textbf{$p=1$ 是黄金标准}。
    \end{itemize}
  \end{keybox}
...
\begin{frame}{级数方法的基本思想}
  ...
  \textbf{常见基函数：}
  \begin{itemize}
    \item \textbf{幂级数 (Power Series)}：$p_k(x) = x^k$——简单但数值病态（高阶幂相关性高）；
